import os, argparse
from ROOT import TH2F, TCanvas, gROOT, TFile, TGraph, TLegend, TLatex, gStyle
from ROOT import kOrange, kGreen, kRed, kAzure, kYellow, kGray
import ROOT
from utiles import GetLumi
gROOT.SetBatch(1)
outputdir = './'
...
def GetDic(pname = 'limits.p', diag = -1, verbose=False):
  import pickle
  ...
  limits['local'] = []; limits['global'] = []
  for  ms, ml, res in results:
    if diag != -1 and not ( (ms - ml == diag) or (ms == 175 and ml == 1)): continue
    limits['mstop'].append(ms); limits['mlsp'].append(ml)
    if len(res) == 2:
      limits['local'].append(res[0])
      limits['global'].append(res[1])
    else:
      ...
      if len(res) >= 6: limits['obs'].append(res[5])
  return limits

def Get2Dhisto(pname = 'limits.p', tag = 'exp'):
  dic = GetDic(pname)
  #print 'Drawing from: ', pname
  mstop = dic['mstop']
  mlsp  = dic['mlsp']
  minstop = min(mstop)
  minlsp  = 0#min(mlsp)
  ...
  h.SetDirectory(0)
  for m in range(minstop, maxstop+10, 10): labelsX.append(str(m))
  for m in range(minlsp, maxlsp+10, 10): labelsY.append(str(m))
  for i in range(0, len(labelsX)):   h.GetXaxis().SetBinLabel(i+1, labelsX[i])
  for i in range(0, len(labelsY)):   h.GetYaxis().SetBinLabel(i+1, labelsY[i])
  return h
...
def Draw2Dhisto(tag = 'exp', pname = 'limits', lim=[0,2.5]):
  ...
  h.Draw('colz')
  for f in ['png', 'pdf']: c.Print('%s.%s'%(pname.replace('limits', 'scan'),f))
...
def Draw2Dsig(tag = 'exp', pname = 'limits', ms=275, ml= 100, lim=[]):
  ...
  g.SetMarkerSize(1); g.SetMarkerStyle(20); 
  gStyle.SetPalette(51)#58)
  h.Draw('colz')
  g.Draw('psame')
  t = TLatex(0,0,'Injected signal %s_%s'%(str(ms),str(ml)))
  t.SetNDC(); t.SetX(0.15); t.SetY(0.85); t.SetTextAlign(12)
  t.Draw()
  for f in ['png', 'pdf']: c.Print('%s.%s'%(pname.replace('limits', 'scan'),f))
...
#varnames = ['metScanstop16000', 'met', 'dnn', 'mt2']
...
#Draw2Dsig('local', 'significance_comb_dnn_225_50', 225, 50)

if args.all:
  varnames = ['dnn']#, 'met']
  years = [2016, 2017, 2018, 'comb']
  chan = ['emu', 'ee', 'mumu', 'all']

if not isinstance(chan, list): chan = [chan]

for ch in chan:
  for var in varnames:
    for year in years:
      pname = 'limits_%s_%s%s'%(str(year),var, ('_%s'%ch) if ch in ['ee', 'mumu', 'all'] else '')
      print 'Drawing plots for: ', pname
      DrawBrazil(diagonal = 175, doData = True, pname = pname, year=year)
      Draw2Dhisto(tag='obs',pname = pname)
#SaveHistos()
#for var in ['metScanstop16000', 'met', 'dnn']: Draw2Dhisto(pname='scan_'+var)
...
#Draw1Dcomp(False, 'limits_dnn.p,limits_met.p,limits_mt2.p')
