...
class JetReCalibrator:
  ...
  class Jet:
    def init(self):
      ...
      self.mass = 0
      self.rawFactor = 0
      self.jetEnergyCorrUncertainty = 0

    def __init__(self, event, ijet):
      self.init()
      self.pt        = event.Jet_pt[ijet]
      self.eta       = event.Jet_eta[ijet]
      self.phi       = event.Jet_phi[ijet]
      self.mass      = event.Jet_mass[ijet]
      self.rawFactor = event.Jet_rawFactor[ijet]
