...
class StackPlot(XuanPlot):
  ...
  def GetOutName(self):
    name = self.histoName[0] if isinstance(self.histoName, list) else self.histoName
    return self.GetOutPath()+name
  ...
  def SetHisto(self, histoName=''):
    if   histoName != '' and isinstance(histoName, str): 
      self.histoName = histoName
  ...
  def SumExpSyst(self, syst = ''):
    ...
    if isinstance(syst, list):
      ...
      return
    if self.verbose >= 2: print 'Adding exp systematic %s...'%syst
    hnom = self.GetAllBkg()
    self.SetSyst(syst)
...
class HistoManager:
  ...
  def GetRatioHistoUnc(self, syst = ''): # syst = stat for stat unc
    ...
    for i in range(0, nb+2):
      ...
      bUnc = hUnc.GetBinError(i)
      bUnc.SetBinContent(i, 1)
      bUnc.SetBinError(i, bUnc/b if b > 0 else 0)
    bUnc.SetFillColorAlpha(kAzure+2, 0.5)
    bUnc.SetFillStyle(1000)
    return bUnc
