...
from framework.fileReader import getDicFiles, GetAllInfoFromFile, IsVarInTree
...
def RunSample(selection, path, sample, year = 2018, xsec = 1, nSlots = 1, outname = '', outpath = '', options = '', nEvents = 0, FirstEvent = 0, prefix = 'Tree', verbose = False, pretend = False, dotest = False, sendJobs = False, queu = 'short'):
  ...
  'from %s import %s'%selection
  evRang = []
  if nEvents != 0: evRang = [FirstEvent, nEvents]
  an = testAnalysis(path, sample, eventRange = evRang, xsec = xsec, nSlots = nSlots, options = options, verbose=verbose)
  ...
  if sendJobs: 
    ...
    if pretend:
      print 'Pretending...'
      return
    an.sendJobs(queue=queue, pretend=pretend)
  else:
    print '>> Running...'
    if pretend:
      print 'Pretending...'
...
if os.path.isfile(fname):
  ...
  for sname in spl:
    ...
    ncores  = nslots[sname]
    RunSample(selection, path, sample, year, xsec, ncores, outname, outpath, options, nEvents, FirstEvent, prefix, verbose, pretend, dotest, sendJobs, queue)

else: # no config file...
  RunSample(selection, path, sample, year, xsec, nSlots, outname, outpath, options, nEvents, FirstEvent, prefix, verbose, pretend, dotest, sendJobs, queue)
