...
class DYDD:
 ...
 def SetMode(self, mode = 'OF'):
   self.doOF = True
   if mode == 'SF' or mode == 'Elec' or mode == 'Muon':
     self.doOF = False
   if   self.doOF: self.SetZhistoName('DY_InvMass')
   else:           self.SetZhistoName('DY_SF_InvMass')
 ...
 def GetKfactor(self, chan = '', level = ''):
   ...
   chan, level = self.SetChanAndLevel(chan, level)
   rinElec, errElec = self.GetDataIn('Elec', level)
   rinMuon, errMuon  = self.GetDataIn('Muon', level)
   k = (rinElec/rinMuon if rinMuon != 0 else 0) if self.GetChan() == 'Elec' else (rinMuon/rinElec if rinElec != 0 else 0)
 ...
 def GetDYDD(self, chan = '', level = '', mode = ''):
   ...
   kee,   keerr    = self.GetKfactor('Elec', level)
   NeeIn, NeeInerr = self.GetDataIn( 'Elec', level)
   kmm  , kmmrr    = self.GetKfactor('Muon', level)
   NmmIn, NmmInerr = self.GetDataIn( 'Muon', level)
 ...
 def GetScaleFactor(self, chan = '', level = '', mode = ''):
   ...
   if chan == 'Elec' or chan == 'Muon':
     ...
     e, er = self.GetScaleFactor('Elec')
     m, mr = self.GetScaleFactor('Muon')
     SF = sqrt(e*m)
 ...
 def PrintDYestimate(self, doSF = False, name = '', level = ''):
   ...
   s('','Elec','Muon','ElMu')
   t.sep()
   mie, miee = self.GetMCin( 'Elec')
   moe, moee = self.GetMCout('Elec')
   mim, mime = self.GetMCin( 'Muon')
   mom, mome = self.GetMCout('Muon')
   s(' N_in        (MC)',v(mie,miee),v(mim,mime),'')
   s(' N_out       (MC)',v(moe,moee),v(mom,mome),'')

   re, ree   = self.GetRoutin('Elec')
   rm, rme   = self.GetRoutin('Muon')
   s(' R_(out/in)  (MC)',v(re,ree),v(rm,rme),'')

   ke, kee   = self.GetKfactor('Elec')
   km, kme   = self.GetKfactor('Muon')
   s(' k_ll            ',v(ke,kee),v(km,kme),'')

   ie, ier   = self.GetDataIn('Elec')
   im, imr   = self.GetDataIn('Muon')
   iem, iemr = self.GetDataIn('ElMu')
   s(' N_in      (Data)',d(ie,ier),d(im,imr),d(iem,iemr))
   t.sep()
   dde, ddee = self.GetDYDD('Elec')
   ddm, ddme = self.GetDYDD('Muon')
   ...
   t.sep()
   sfee, sfeee = self.GetScaleFactor('Elec')
   sfmm, sfmme = self.GetScaleFactor('Muon')
   if doSF:
 ...
 def PrintDYSFnjets(self):
   ...
   for i in range(len(labels)):
     ...
     lab = labels[i]
     lev = levels[i]
     sfe, sfee = self.GetScaleFactor('Elec', lev, 'SF')
     sfm, sfme = self.GetScaleFactor('Muon', lev, 'SF')
     sfo, sfoe = self.GetScaleFactor('ElMu', lev, 'OF')
     s(' '+lab, v(sfe,sfee) if sfe > 0 else '-', v(sfm,sfme) if sfm > 0 else '-', v(sfo,sfoe) if sfo > 0 else '-')
   t.bar()
   t.write()

 def __init__(self, path, outpath = './temp/', chan = 'ElMu', level = '2jets', DYsamples = 'DYJetsToLL_M_10to50,DYJetsToLL_MLL50', DataSamples = 'HighEGJet,SingleMuon, DoubleMuon', lumi = 308.54, massRange = [91-15, 91+15], mode = '', histonameprefix = 'H'):
   self.SetPath(path)
