...
class NonpromptDD:
 ...
 def PrintSSyields(self, name = '', level = ''):
   ...
   S = lambda tit, pr : s(tit, v(self.GetSS(pr,'Elec')), v(self.GetSS(pr, 'Muon')), v(self.GetSS(pr, 'ElMu')))
   D = lambda tit, pr : s(tit, d(self.GetSS(pr,'Elec')), d(self.GetSS(pr, 'Muon')), d(self.GetSS(pr, 'ElMu')))
   s('','Elec','Muon','ElMu')
   t.sep()
 ...
 ### Init
 def __init__(self, path, outpath = './temp/', chan = 'ElMu', level = '2jets', process = {}, prompt = [], nonprompt = [], lumi = 308.54, histonameprefix = 'H'):
   self.SetPath(path)
